#' @noRd
genai.openai.chat.history.print = function(genai.openai.object,
                                           from,
                                           to) {
  ...
  chat.length = length(genai.openai.object$chat.history$messages)
  ...
  if (chat.length > 0) {
    for (i in from:chat.length) {
      ...
      cat("Role:",
          genai.openai.object$chat.history$messages[[i]]$role,
          "\n")
      cat("Text: ")
      cat(paste(strwrap(genai.openai.object$chat.history$messages[[i]]$content,
