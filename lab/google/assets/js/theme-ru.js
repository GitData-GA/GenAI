...
async function fetchUserCountryName() {
    try {
        ...
        if (typeof Intl?.DisplayNames === "function") {
            try {
                const enNames = new Intl.DisplayNames(["ru"], {
                    type: "region"
                });
...
const updateProxyStatusFooter = () => {
    if (proxyToggle.checked) {
        proxyStatusFooter.innerHTML = `Использование публичного прокси-сервера. Ознакомьтесь с <a href="https://genai.gd.edu.kg/api/" target="_blank" class="text-blue-500 hover:underline">рисками и условиями</a>.`;
    } else {
...
const showWelcomeMessage = () => {
    chatMessages.innerHTML = `
        <div class="welcome-container">
            <h1 class="text-xl font-semibold welcome-text">Добро пожаловать в лабораторию GitData GenAI для Google Gemini!</h1>
            <p class="mt-2 welcome-text">Не стесняйтесь спрашивать что угодно!</p>
        </div>
    `;
...
const showTypingIndicator = () => {
    ...
    typingElement.innerHTML = `<div class="flex-shrink-0"><img src="https://static.gd.edu.kg/images/logo.svg" alt="Bot" width="32px" /></div><div class="flex-1 p-4 rounded-xl message-bubble flex items-center"><div class="typing-indicator">Мышление <span></span><span></span><span></span></div></div>`;
...
const callGeminiApi = async (text, images) => {
    ...
    const payload = {
        system_instruction: {
          parts: [
            {
              text: "Вы дружелюбный и отзывчивый помощник. Следите за полнотой своих ответов, если только пользователь не просит более краткого ответа. При генерации кода при необходимости поясняйте фрагменты кода и придерживайтесь принципов хорошего программирования. На запросы, требующие информации, давайте ответы, отражающие глубокое понимание предметной области, гарантируя их корректность. На любые запросы, не относящиеся к русскому языку, отвечайте на том же языке, на котором была подсказка, если пользователь не указал иное. В подсказках, требующих рассуждений, чётко объясняйте каждый этап процесса рассуждений, прежде чем представить окончательный ответ."
            }
          ]
        },
    ...
    try {
        ...
        renderContent(errorContainer, `Извините, что-то пошло не так.\n\n**Ошибка:**\n\`\`\`\n${error.message}\n\`\`\``);
        scrollToBottom();
    }
};
...
clearStorageButton.addEventListener('click', () => {
    if (confirm('Вы уверены, что хотите удалить все данные? Это приведет к удалению вашего API-ключа и всей истории чатов.')) {
        localStorage.clear();
        location.reload();
    }
});
